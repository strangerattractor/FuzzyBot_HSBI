...
def retrieve_context(query: str,
                     top_k: int = RAG_TOP_K,
                     max_chars: int = RAG_MAX_CHARS) -> str:
    """
    Retrieve relevant context from LanceDB for a given query.
    Returns a concatenated text block or "" if RAG is disabled/empty.
    """
    ...
    try:
        ...
        for idx, h in enumerate(hits):
            prefix = f"[{h.get('doc_id', 'unknown')} p.{h.get('page', '?')}] "
            text = str(h.get("text", ""))
            snippet = prefix + text
            ...
            if RAG_DEBUG >= 1:
                ...
                print(f"[RAG]  -> hit {idx}: {h.get('doc_id')} p.{h.get('page')} "
...
def apply_rag_to_messages(messages: List[ChatMessage]) -> List[ChatMessage]:
    """
    ...
    Returns a new list of ChatMessage.
    """
    if not _RAG_ENABLED:
        if RAG_DEBUG:
...
def chat(req: ChatRequest):
    ...
    # You can also pipe this through RAG if you want:
    messages = [ChatMessage(role="user", content=req.prompt)]
    messages = apply_rag_to_messages(messages)
...
async def v1_chat_completions(req: ChatCompletionRequest):
    ...
    rag_messages = apply_rag_to_messages(req.messages)
